...
import 'package:solidcare/model/configuration_model.dart';
...
Future<void> getConfigurationAPI() async {
  ...
  appStore.setUserProEnabled(value.isSolidCareProOnName.validate(),
      initialize: true);
  appStore.setGoogleMeetEnabled(value.isSolidCareGoogleMeetActive.validate());
  ...
  appStore.setRestrictAppointmentPost(
      value.restrictAppointment!.post.validate(value: '365').toInt(),
      initialize: true);
  appStore.setRestrictAppointmentPre(
      value.restrictAppointment!.pre.validate().toInt(),
      initialize: true);
...
Future<void> saveLanguageApi(Map<String, dynamic> request) async {
  await (handleResponse(
      await buildHttpResponse(ApiEndPoints.saveLanguageApiEndPoint)));
}
