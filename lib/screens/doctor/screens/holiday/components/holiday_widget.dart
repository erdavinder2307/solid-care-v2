...
class HolidayWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    int totalDays = getDateDifference(data.holidayStartDate.validate(), eDate: data.holidayEndDate.validate(), isForHolidays: true);
            ...
            children: [
              Column(
                ...
                children: [
                      ...
                      if (data.userProfileImage.validate().isNotEmpty)
                        ...
                        GradientBorder(
                          ...
                          child: PlaceHolderWidget(
                            ...
                            child: Text(isDoctor() ? data.userName.validate(value: 'D')[0].capitalizeFirstLetter() : data.userName.validate(value: 'C')[0].capitalizeFirstLetter(),
                  ...
                  10.height,
                  if (isOnLeave)
                    Text(data.userName.split(' ').first.validate() + " " + locale.lblIsOnLeave, style: boldTextStyle(size: 14, color: appPrimaryColor))
                  else
                    Text(locale.lblAfter + ' ${pendingDays == 0 ? '1' : pendingDays.abs()} ' + locale.lblDays, style: boldTextStyle(size: 14)).visible(isPending),
                  if (!isOnLeave)
                    Text(
                      locale.lblWasOffFor + ' ${totalDays == 0 ? '1' : totalDays} ' + locale.lblDays,
