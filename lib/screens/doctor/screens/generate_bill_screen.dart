...
class _GenerateBillScreenState extends State<GenerateBillScreen> {
    ...
    if (multiSelectStore.taxData != null) {
      subTotalCont.text = billItemData
          .sumByDouble((element) =>
              ...
              element.qty.validate().toInt())
          .toString();
      totalCont.text = ((billItemData.sumByDouble((element) =>
                  element.price.validate().toDouble() *
                  element.qty.validate().toInt())) +
  ...
  void setTotalPayable(String v) {
    ...
    if (multiSelectStore.taxData != null) {
      subTotalCont.text =
          '${(billItemData.sumByDouble((element) => element.price.validate().toDouble() * element.qty.validate().toInt())) - discount}';
      payableCont.text =
          '${(billItemData.sumByDouble((element) => element.price.validate().toDouble() * element.qty.validate().toInt()) - discount) + multiSelectStore.taxData!.totalTax.validate()}';
    } else {
      if (v.isNotEmpty) {
