...
import 'package:solidcare/screens/doctor/components/multi_select_clinic_drop_down.dart';
...
class _AddServiceScreenState extends State<AddServiceScreen> {
  ...
  void setInitialData() {
    if (isReceptionist()) {
      ...
      if (isUpdate) {
        selectedDoctorData = userStore.userData != null
            ? userStore.userData
          ...
          if (isDoctor()) {
            if (selectedDataList.validate().isNotEmpty) {
              selectedDataList.forEachIndexed((element, index) {
                ...
                req.putIfAbsent(
                    'doctors[$index][doctor_id]', () => userStore.userId);
                req.putIfAbsent(
    ...
    return Observer(builder: (context) {
      return Scaffold(
        appBar: appBarWidget(
          ...
          actions: [
            if (isUpdate &&
                isVisible(SharedPreferenceKey.solidCareServiceDeleteKey))
              IconButton(
                icon: Icon(
                  Icons.delete,
                  color: Colors.white,
          ...
          onPressed: () {
            if (isUpdate && listOfMappingTableId.isNotEmpty)
              ...
              if (isDoctor() && isUpdate) {
                if (selectedDataList.validate().first.clinicId !=
                    widget.serviceData!.clinicId) {
                  if (widget.serviceList.validate().any((element) =>
                      element.clinicId == selectedDataList.first.clinicId &&
                      element.serviceId == selectedDataList.first.serviceId)) {
